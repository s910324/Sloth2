...
class TableWidget(QTableView):
	...
	def enable_forzen_view(self, row_index, column_index):
		print (column_index)
		if  row_index:
			self.top_frozen_view    = QTableView(self)
		if  column_index:
			self.left_frozen_view   = QTableView(self)
			self.left_frozen_view.setStyleSheet('''border: none;''')			
			self.left_frozen_view.setHorizontalScrollMode(QAbstractItemView.ScrollPerPixel)
			self.left_frozen_view.setVerticalScrollMode(  QAbstractItemView.ScrollPerPixel)
			self.viewport().stackUnder(self.left_frozen_view)
			self.verticalScrollBar().valueChanged.connect(self.left_frozen_view.verticalScrollBar().setValue)
			self.left_frozen_view.verticalScrollBar().valueChanged.connect(self.verticalScrollBar().setValue)       
			self.left_frozen_view.setModel(self.model())
			self.left_frozen_view.verticalHeader().hide()
			self.left_frozen_view.setFocusPolicy(Qt.NoFocus)
			self.left_frozen_view.horizontalHeader().setSectionResizeMode(QHeaderView.Fixed)
			self.left_frozen_view.setSelectionModel(self.selectionModel())
			self.left_frozen_view.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
			self.left_frozen_view.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)			

		if  column_index and  row_index:
			self.corner_frozen_view = QTableView(self)
	...
	def updateSectionWidth(self, logicalIndex, oldSize, newSize):
		if logicalIndex==0 or logicalIndex==1:
			self.left_frozen_view.setColumnWidth(logicalIndex, newSize)
			self.updateFrozenTableGeometry()
 
	def updateSectionHeight(self, logicalIndex, oldSize, newSize):
		self.left_frozen_view.setRowHeight(logicalIndex, newSize)
	...
	def moveCursor(self, cursorAction, modifiers):
		...
		if cursorAction == self.MoveLeft and current.column() > 1 and self.visualRect(current).topLeft().x() < (self.left_frozen_view.columnWidth(0) + self.left_frozen_view.columnWidth(1)):
			newValue = self.horizontalScrollBar().value() + self.visualRect(current).topLeft().x() - (self.left_frozen_view.columnWidth(0) + self.left_frozen_view.columnWidth(1))
			self.horizontalScrollBar().setValue(newValue)
		return current
		
	def updateFrozenTableGeometry(self):
		if self.verticalHeader().isVisible():
			self.left_frozen_view.setGeometry(
				self.verticalHeader().width() + self.frameWidth(),
				self.frameWidth(), 
				self.columnWidth(0) + self.columnWidth(1),
				self.viewport().height() + self.horizontalHeader().height())
		else:
			self.left_frozen_view.setGeometry(self.frameWidth(),
	...
	def update_data_model(self):
		...
		for col in range(self.data_model.columnCount(self)):
			if col not in [0, 1]:
				print("aa")
				self.left_frozen_view.setColumnHidden(col, True)
